    ...
    <Layout>
      ...
      <article className="py-12 md:py-16">
        <div className="container max-w-4xl">
          ...
          <ArticleSection title={data?.sections?.disinfectObjects?.title}>
            <BulletList items={(data?.sections?.disinfectObjects?.items as string[]) ?? []} />
          </ArticleSection>
          ...
          <ArticleSection title={data?.sections?.power?.title}>
            <Paragraph>{data?.sections?.power?.text ?? ''}</Paragraph>

            <DataTable
              caption={data?.sections?.power?.tableCaption}
              headers={data?.sections?.power?.tableHeaders ?? []}
              rows={data?.sections?.power?.tableData ?? []}
            />

            <HighlightBox variant="success">{data?.sections?.power?.note ?? ''}</HighlightBox>
          </ArticleSection>
          ...
          <ArticleSection title={data?.sections?.comparison?.title}>
            <ComparisonTable
              ...
              headers={
                (data?.sections?.comparison?.tableHeaders ?? []).slice(0, 3) as [
                  string,
                  string,
                  string,
                ]
